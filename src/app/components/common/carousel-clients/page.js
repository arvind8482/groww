...
const CarouselClients = ({ carousalData }) => {
  ...
  const goToNext = () => {
    setCurrentIndex((prevIndex) => {
      const maxIndex = carousalData.length - Math.ceil(window.innerWidth / slideWidth);
  ...
  return (
    <div className="relative w-full overflow-hidden" ref={carouselRef}>
      ...
      <div className='flex justify-center pt-6'>
        ...
        <button 
          onClick={goToNext}
          disabled={currentIndex >= carousalData.length - Math.ceil(window.innerWidth / slideWidth)} 
          className={`ms-2 ${currentIndex >= carousalData.length - Math.ceil(window.innerWidth / slideWidth) ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
